...
export const Overlay = ({ onClose, children, wide }) => (
  <motion.div
    ...
    animate={{ opacity: 1 }}
    exit={{ opacity: 0 }}
    className="fixed inset-0 bg-slate-900/60 flex items-center justify-center z-[1000] p-5 backdrop-blur-sm"
    onClick={onClose}
  >
    <motion.div
...
export const FLabel = ({ label, children, span2 }) => (
  ...
  >
    <label className="text-[10px] font-bold text-gray-400 tracking-[1.2px] uppercase">
      {label}
    </label>
    {children}
  </div>
...
export const DeleteModal = ({ label, onConfirm, onClose, isLoading }) => (
  <Overlay onClose={onClose}>
    <div className="px-8 py-10 text-center">
      ...
      <h2 className="font-display text-[19px] text-slate-900 mb-2.5 font-bold">
        Confirm Delete
      </h2>
      ...
      <div className="flex gap-2.5 justify-center">
        <button
          onClick={onConfirm}
          disabled={isLoading}
          className={`px-7 py-2.5 rounded-xl border-none text-sm font-bold transition-all ${isLoading ? "bg-red-200 text-white cursor-not-allowed opacity-60" : "bg-red-600 hover:bg-red-700 text-white cursor-pointer shadow-lg shadow-red-600/20"}`}
        >
        ...
        <button
          onClick={onClose}
          disabled={isLoading}
          className={`px-7 py-2.5 rounded-xl border border-slate-200 bg-slate-50 text-gray-400 text-sm font-semibold transition-all ${isLoading ? "cursor-not-allowed" : "hover:bg-slate-100 hover:text-gray-600 cursor-pointer"}`}
        >
          Cancel
        </button>
      </div>
