  ...
  const filtered = useMemo(() => {
    let list = events.filter((e) => {
      ...
      const matchesStatus =
        ...
        (statusFilter.includes("Upcoming events") &&
          e.status === "Upcoming Event");
      const matchesMode =
        modeFilter.length === 0 || modeFilter.includes(e.mode);
    ...
    <div className="bg-slate-50 min-h-screen">
      ...
      <section className="bg-slate-900 pt-28 pb-14">
        <div className="max-w-7xl mx-auto px-6 lg:px-10">
          ...
          >
            Chapter launches, campus meets, and the congress that started it all
            &mdash; browse everything the association has hosted.
          </motion.p>
        </div>
      </section>
            ...
            <div className="bg-white border border-slate-100 rounded-2xl p-5">
              ...
              <div className="flex flex-col gap-3">
                {eventStatuses.map((s) => (
                  <label
                    key={s}
                    className="flex items-center gap-2.5 text-sm text-slate-500 cursor-pointer"
                  >
                    <input
                      type="checkbox"
                ...
                <motion.div key={event.id} variants={fadeUp}>
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0 flex flex-col justify-between py-1">
                      ...
                      <span className="mt-4 sm:mt-0 text-xs font-medium text-slate-400">
                        {event.status}
                      </span>
                    </div>
                  </Link>
